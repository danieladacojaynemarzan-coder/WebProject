...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getExpiryStatus(expiry) {
    ...
    if (daysUntilExpiry < 0) {
      ...
      return 'Expired'; // Treat exact expiry day as expired, or adjust if you want it under soon to expire
  ...
  function getStatusLabel(item) {
    ...
    // 1. Critical takes absolute precedence (0 stock)
    if (stock <= 0) return 'Critical';

    // 2. Check Expiry states next
    const expiryStatus = getExpiryStatus(item.expiry);
    if (expiryStatus) return expiryStatus;

    // 3. Low stock check (e.g. 1 to 5 or 6 items)
  ...
  function getStatusClass(item) {
    ...
    if (expiryStatus === 'Soon to expire') return 'expire';
  ...
  function renderInventory() {
    ...
    pageItems.forEach((item, index) => {
      ...
      row.innerHTML = `
        <td>${displayIndex}</td>
        <td><span style="color: #000000 !important; font-weight: 500;">${escapeHtml(item.name || '')}</span></td>

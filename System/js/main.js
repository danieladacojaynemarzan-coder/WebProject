    ...
    try {
      function loadHeaderProfile() {
        try {
          ...
          const isMale = !!(profile && (profile.sex === 'Male' || profile.gender === 'Male'));
          ...
          document.querySelectorAll('.nurse-avatar, .header-avatar, .profile-avatar').forEach((el) => {
            ...
            } else if (el.classList.contains('nurse-avatar')) {
              el.innerHTML = isMale ? '👨‍⚕️' : '👩‍⚕️';
            } else {
              el.innerHTML = isMale ? '<i class="fa-solid fa-user-doctor"></i>' : '<i class="fa-solid fa-user-nurse"></i>';
            }
          });
